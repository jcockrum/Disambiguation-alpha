require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe SocietiesController do

  # This should return the minimal set of attributes required to create a valid
  # Society. As you add validations to Society, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    { "structure" => "MyString" }
  end

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # SocietiesController. Be sure to keep this updated too.
  def valid_session
    {}
  end

  describe "GET index" do
    it "assigns all societies as @societies" do
      society = Society.create! valid_attributes
      get :index, {}, valid_session
      assigns(:societies).should eq([society])
    end
  end

  describe "GET show" do
    it "assigns the requested society as @society" do
      society = Society.create! valid_attributes
      get :show, {:id => society.to_param}, valid_session
      assigns(:society).should eq(society)
    end
  end

  describe "GET new" do
    it "assigns a new society as @society" do
      get :new, {}, valid_session
      assigns(:society).should be_a_new(Society)
    end
  end

  describe "GET edit" do
    it "assigns the requested society as @society" do
      society = Society.create! valid_attributes
      get :edit, {:id => society.to_param}, valid_session
      assigns(:society).should eq(society)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Society" do
        expect {
          post :create, {:society => valid_attributes}, valid_session
        }.to change(Society, :count).by(1)
      end

      it "assigns a newly created society as @society" do
        post :create, {:society => valid_attributes}, valid_session
        assigns(:society).should be_a(Society)
        assigns(:society).should be_persisted
      end

      it "redirects to the created society" do
        post :create, {:society => valid_attributes}, valid_session
        response.should redirect_to(Society.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved society as @society" do
        # Trigger the behavior that occurs when invalid params are submitted
        Society.any_instance.stub(:save).and_return(false)
        post :create, {:society => { "structure" => "invalid value" }}, valid_session
        assigns(:society).should be_a_new(Society)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Society.any_instance.stub(:save).and_return(false)
        post :create, {:society => { "structure" => "invalid value" }}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested society" do
        society = Society.create! valid_attributes
        # Assuming there are no other societies in the database, this
        # specifies that the Society created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Society.any_instance.should_receive(:update_attributes).with({ "structure" => "MyString" })
        put :update, {:id => society.to_param, :society => { "structure" => "MyString" }}, valid_session
      end

      it "assigns the requested society as @society" do
        society = Society.create! valid_attributes
        put :update, {:id => society.to_param, :society => valid_attributes}, valid_session
        assigns(:society).should eq(society)
      end

      it "redirects to the society" do
        society = Society.create! valid_attributes
        put :update, {:id => society.to_param, :society => valid_attributes}, valid_session
        response.should redirect_to(society)
      end
    end

    describe "with invalid params" do
      it "assigns the society as @society" do
        society = Society.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Society.any_instance.stub(:save).and_return(false)
        put :update, {:id => society.to_param, :society => { "structure" => "invalid value" }}, valid_session
        assigns(:society).should eq(society)
      end

      it "re-renders the 'edit' template" do
        society = Society.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Society.any_instance.stub(:save).and_return(false)
        put :update, {:id => society.to_param, :society => { "structure" => "invalid value" }}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested society" do
      society = Society.create! valid_attributes
      expect {
        delete :destroy, {:id => society.to_param}, valid_session
      }.to change(Society, :count).by(-1)
    end

    it "redirects to the societies list" do
      society = Society.create! valid_attributes
      delete :destroy, {:id => society.to_param}, valid_session
      response.should redirect_to(societies_url)
    end
  end

end
